...
def _invite_secret() -> str:
    if not _SECRET_KEY or len(_SECRET_KEY.encode("utf-8")) < 32:
        logger.error("[FAMILY] Invite signing secret is missing or too short.")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Family invitations are temporarily unavailable.",
        )
    return _SECRET_KEY

def _create_invite_token(primary_user_id: int, nonce: str) -> str:
    ...
    return jwt.encode(payload, _invite_secret(), algorithm=_ALGORITHM)


def _decode_invite_token(token: str) -> tuple[int, str]:
    """Decode and validate an invite token.  Returns the primary user's ID.

    Raises HTTP 400 on any tampering, expiry, or wrong ``typ``.
    """
    try:
        payload = jwt.decode(token, _invite_secret(), algorithms=[_ALGORITHM])
    ...
    try:
        primary_id = int(payload["sub"])
        nonce = payload["jti"]
        if not isinstance(nonce, str) or not nonce:
            raise ValueError("missing invitation nonce")
        return primary_id, nonce
...
def generate_invite_code(
    ...
    nonce = uuid4().hex
    current_user.family_invite_nonce = nonce
    current_user.family_invite_expires_at = datetime.now(timezone.utc) + timedelta(
        hours=_INVITE_TTL_HOURS
    )
    db.commit()
    token = _create_invite_token(current_user.id, nonce)
...
def join_family(
    ...
    primary_id, invite_nonce = _decode_invite_token(body.invite_code)
    ...
    invite_expiry = primary_user.family_invite_expires_at
    if invite_expiry is not None and invite_expiry.tzinfo is None:
        invite_expiry = invite_expiry.replace(tzinfo=timezone.utc)
    if (
        primary_user.family_invite_nonce != invite_nonce
        or invite_expiry is None
        or invite_expiry <= datetime.now(timezone.utc)
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    primary_user.family_invite_nonce = None
    primary_user.family_invite_expires_at = None
